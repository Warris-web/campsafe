...
// Configurable via .env — default 5 minutes (was 2, too trigger-happy for normal signal gaps)
const MISSING_THRESHOLD_MS = (parseInt(process.env.MISSING_THRESHOLD_MINUTES) || 5) * 60 * 1000;
  ...
  setInterval(async () => {
    try {
      ...
      for (const { _id: device_id, lastSeen, doc } of latest) {
        ...
        if (isMissing && !alreadyMissing.has(device_id)) {
          alreadyMissing.add(device_id);
          io.emit("tracker_missing", { device_id, lastSeen, ageMinutes: Math.floor(age / 60000), lastPosition: doc });
          console.warn(`⚠️  MISSING: ${device_id} — last seen ${Math.floor(age / 60000)}m ago`);
        }
